...
export const CarouselControls: React.FC<Props> = ({ mode, currentImage, length, carouselControl }) => {
    ...
    return (
        <>
            <button className={styles.next} onClick={() => { carouselControl('forward') }} disabled={mode === 'standard' && currentImage + 1 === length}>
                <figure className={styles.next_icon} />
            </button>
            <button className={styles.previous} onClick={() => { carouselControl('backward') }} disabled={mode === 'standard' && currentImage === 0}>
