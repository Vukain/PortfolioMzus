...
export const fetchEntries = cache(async () => {
  const imageLists = (await client.getEntries({ content_type: 'image-list' }).then((response) => {
    return response.items;
  })) as Array<ContentfulImages>;
  const videoLists = (await client.getEntries({ content_type: 'video-list' }).then((response) => {
    return response.items;
  })) as Array<ContentfulVideos>;
