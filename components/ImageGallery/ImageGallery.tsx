...
export const ImageGallery: React.FC<Props> = ({ images, columns = 3 }) => {
  ...
  const generateImage = (item: CloudinaryImage, index: number) => {
    ...
    return (
      <div className={styles.wrapper} key={index} onClick={(e) => onClickHandler(e, index)}>
        <img
    ...
    if (images?.content.length > 0) {
      if (columns > 1 && isMasonryEnabled) {
        return splitIntoChunks(columns, images.content).map((chunk, index) => {
          ...
          return (
            <div key={index} className={styles.masonry_column}>
              {column}
            </div>
          );
        });
  ...
  return (
    <>
      <PageTransition>
        <div className={styles.gallery}>{generateContent()}</div>
      </PageTransition>
      {showCarousel && (
        <InfiniteCarousel
